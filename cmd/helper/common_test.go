...
func Test_commonHelper_Write(t *testing.T) {
	type fields struct {
		...
		shellRepo      *repository.ShellRepoMock
		fileSystemRepo *repository.FileSystemRepoMock
	}
	type args struct {
		templatePath string
		outputPath   string
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{},
			prepare: func(f *fields) {
				f.fileSystemRepo.On("CreateDirectory", mock.Anything).Return(nil)
				f.fileSystemRepo.On("WriteFile", mock.Anything, mock.Anything).Return(nil)
			},
			args: args{
		...
		},
		{
			...
			fields: fields{},
			prepare: func(f *fields) {
				f.fileSystemRepo.On("CreateDirectory", mock.Anything).Return(errors.New("error"))
			},
			args: args{
				templatePath: "test.tmpl",
				outputPath:   "./test.go",
		...
		},
		{
			...
			prepare: func(f *fields) {
				f.fileSystemRepo.On("CreateDirectory", mock.Anything).Return(nil)
				f.fileSystemRepo.On("WriteFile", mock.Anything, mock.Anything).Return(errors.New("error"))
			},
			args: args{
				templatePath: "test.tmpl",
				outputPath:   "./test.go",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.fields = fields{
				...
				shellRepo:      repository.NewShellRepoMock(t),
				fileSystemRepo: repository.NewFileSystemRepoMock(t),
			}
...
func Test_commonHelper_ExecShell(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{},
			prepare: func(f *fields) {
				f.shellRepo.On("Exec", mock.Anything).Return([]byte("test.txt"), nil)
			},
			args: args{
				command: "ls",
			},
		...
		},
		{
			...
			fields: fields{},
			prepare: func(f *fields) {
				f.shellRepo.On("Exec", mock.Anything).Return([]byte(""), errors.New("ls no such command found"))
			},
			args: args{
				command: "ls",
			},
